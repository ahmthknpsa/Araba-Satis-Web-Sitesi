document.addEventListener("DOMContentLoaded", async function () {
    ...
    async function fetchTruckCars() {
      carList.innerHTML = "<p>Veriler yükleniyor...</p>";
  
      try {
        const response = await fetch("http://localhost:5024/api/cars/filter?kasaTipi=Truck");
        const cars = await response.json();
  
        carList.innerHTML = ""; // Önceki içeriği temizle
  
        if (cars.length === 0) {
          carList.innerHTML = "<p>Hiç Truck araç bulunamadı.</p>";
          return;
        }
    ...
    fetchTruckCars(); // Truck araçlarını getir
  });
