document.addEventListener("DOMContentLoaded", async function () {
    ...
    async function fetchSuvCars() {
      carList.innerHTML = "<p>Veriler yükleniyor...</p>";
  
      try {
        const response = await fetch("http://localhost:5024/api/cars/filter?kasaTipi=SUV");
        const cars = await response.json();
  
        carList.innerHTML = ""; // Önceki içeriği temizle
  
        if (cars.length === 0) {
          carList.innerHTML = "<p>Hiç SUV araç bulunamadı.</p>";
          return;
        }
    ...
    fetchSuvCars(); // SUV araçlarını getir
  });
